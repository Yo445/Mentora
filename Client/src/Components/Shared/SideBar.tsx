...
import { getAccessToken, getAuthUser, getRefreshToken, removeAuthUser } from "../../helper/Storage";
...
export default function SideBar({
  ...
  // Define states for user roles
  const [isInstructor, setIsInstructor] = useState<boolean>(false);
  const [isStudent, setIsStudent] = useState<boolean>(false);

  useEffect(() => {
    if (authUser) {
      setIsInstructor(authUser.role === "instructor");
      ...
      <div className="fixed w-full z-30 flex bg-white dark:bg-[#202330] p-2 items-center justify-center h-16 px-10">
        ...
        <div className="flex-none h-full text-center flex items-center justify-center">
          <div className="flex space-x-3 items-center px-3">
            <button
              onClick={Logout}
              className="hover:ml-4 justify-end pr-5 text-white dark:hover:text-[#ddff7d] w-full p-3 rounded-full flex"
            >
        ...
        >
          {isStudent && (
            <>
              <Link
                to="students/"
                className="hover:ml-4 w-full text-white dark:hover:text-[#ddff7d] bg-[#000] p-2 pl-8 rounded-full flex flex-row items-center space-x-3"
              >
